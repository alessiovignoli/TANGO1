...
def fasta_longest(in_fasta):
	with open(in_fasta, 'r') as infasta:
		...
		for line in infasta:
			if line[0] == '>':
				if len( buffer_seq ) > len( sequence ):		## using the sequence saved itself as variable for the max length found
					sequence = buffer_seq
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='picks the longest sequence and corresponding header present in a fasta file and prints it to standardout')
