...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="This script has 4 inputs: a bulk GTex data of RNA-seq experiments for example, a python dictionary file saved with pickle containing all the tissue - sample ID relationships, third a transcript/gene/exon ecc .. ID and fourth another ID findable in the bulk data as well. With all this info the script will compute average ratio expression levels for the given IDs pair for each tissue type found in the dict. First by doing the ratio (dividing) the same sample values from the 2 IDS, (ID1_smaple1_val / ID2_sample1_val), it also doeas the log of this as additional information. Second it sums all the ratios (log and non) from the same Tissue divided by the number encountered for that tissue (minus those for whom the ratio is not computable 0/n or n/0). Since this script is developed to handle GTex v8 data, it relies on the fact that there is a header line (specifiable) that contains the sample ID present in the Dict, and that the column identified by this sample ID alway contains values associated with it. Basically that column have a meaning related to the sample ID.")
    ...
    parser.add_argument("-id1", "--ID1", type=str, required=True, metavar="STR", help="The first query ID that have to be extracted from the above file. Numerator in the ratio")
    parser.add_argument("-id2", "--ID2", type=str, required=True, metavar="STR", help="The second query ID that have to be extracted from the above file. Denominator in the ratio.")
    parser.add_argument("-o", "--out_name", type=str, required=True, metavar="FILE", help='The path to where to save the output. It will be in the format of the delimiter specified. It will have the following fields:\nTissue + delimiter + average log-ratio + delimiter + average ratio + delimiter + number of samples')
    ...
    parser.add_argument("-hl", "--header_line", type=int, required=False, nargs='?', const=1, default=1, metavar="POS", help='The very important line in bulk data that contains the sample_IDs present as values of the dictionary. From this line the values found in the line of the query Id are correlated (through  the dictionary) to the tissue. first line = 0, default 1 ')
