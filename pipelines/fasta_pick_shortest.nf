...
if (params.help) {
        log.info "This pipeline picks the shortest fasta sequence and corresponding header and saves it to a given file"
        log.info "in case of multiple input fasta files it picks the shortest from each one of them and saves to one output file"
        log.info ""
        ...
        log.info "		in case of many files the shortest sequence is picked from each one"
...
workflow  fasta_shortest {
	...
	fasta_pick_the_shortest_pyscript = params.SCRIPTS + "fasta_pick_the_shortest.py"
	fasta_shortest_picker(fastas, fasta_pick_the_shortest_pyscript)
	out_filename = "${pattern_to_out}".split('/')[-1]
	multifasta = fasta_shortest_picker.out.tmp_files.collectFile(name: out_filename, storeDir: params.OUT_DIR)

	emit:
	//stout = fasta_shortest_picker.out.standardout 
	multifasta
}

workflow {
	fasta_shortest(params.IN, params.OUT)
	//fasta_shortest.out.stout.view()		// for debug
	fasta_shortest.out.multifasta.view()
}
